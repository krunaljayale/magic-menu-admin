...
function LiveOrderCard({ id }: LiveOrderCardProps) {
  ...
  const fetchOrderData = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API.GET_LIVE_ORDER_DATA}/${id}`, {
        headers: {
      ...
      if (!response.ok) throw new Error("Failed to fetch live order data");

      const json = await response.json();
      setData(json.data); // ✅ API returns { success, data }
    } catch (err: any) {
      console.error("[GET /get-live-order-data/:id] Error:", err);
